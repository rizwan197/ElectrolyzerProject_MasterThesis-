function [lbz,lbx,lbu,ubz,ubx,ubu] = decision_varboundEdit(N)
...
lbT_cw_out = 10*ones(N,1);       %lower bound on the coolant outlet temperature
...
% lbq_lye_k = 6.59e3*ones(N,1);      %fixed lye flowrate for OSHex
% ubq_lye_k = 6.59e3*ones(N,1);

lbq_lye_k = 7.02e3*ones(N,1);      %fixed lye flowrate for NewHex
ubq_lye_k = 7.02e3*ones(N,1);

% lbq_lye_k = 6.49e3*ones(N,1);      %fixed lye flowrate for DegHex
% ubq_lye_k = 6.49e3*ones(N,1);

lbQcool = 0*ones(N,1);        %lower bound on the coolant flow rate
ubQcool = (8e4/N)*par.Const.Cp*70*ones(N,1);

lbqH2O = 0*ones(N,1);           %lower bound on total water lost during electrolysis
ubqH2O = inf*ones(N,1);

lbz = [lbu_k;lbi_k;lbP_k;lbFeff_k;lbnH2_k;lbqH2Oloss_k;lbT_bt_in;lbT_el_in;lbT_cw_out];          %lower bounds on all the algebriac variables
lbx = [lbT_k;lbMbt;lbT_bt_out];                                                                  %lower bounds on all the differential variables
lbu = [lbU_el_k;lbq_lye_k;lbQcool;lbqH2O];                                                       %lower bounds on all the input variables
ubz = [ubu_k;ubi_k;ubP_k;ubFeff_k;ubnH2_k;ubqH2Oloss_k;ubT_bt_in;ubT_el_in;ubT_cw_out];          %upper bounds on all the algebriac variables
ubx = [ubT_k;ubMbt;ubT_bt_out];                                                                  %upper bounds on all the differential variables
ubu = [ubU_el_k;ubq_lye_k;ubQcool;ubqH2O];                                                       %upper bounds on all the input variables
