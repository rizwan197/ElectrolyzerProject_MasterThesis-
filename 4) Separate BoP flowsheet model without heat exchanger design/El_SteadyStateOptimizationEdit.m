function [z0, x0, u0, EXIT] = El_SteadyStateOptimizationEdit(N,X0,P0)

import casadi.*
par = parElectrolyzer(N);

%% Build the plant model and solve steady state optimization problem
[xDiff, xAlg, input, eqnAlg, eqnDiff] = modelEdit(par.N);
x = [xAlg;xDiff];
...
[lbz,lbx,lbu,ubz,ubx,ubu] = decision_varboundEdit(par.N);
lbw = [lbw;lbz;lbx;lbu];
ubw = [ubw;ubz;ubx;ubu];
...
deltaT1_k = xAlg(7*par.N+1:8*par.N) - par.Tw_in_k*ones(par.N,1);   %difference in temperatures of hot and cold streams at inlet
deltaT2_k = xDiff(2*par.N+1:3*par.N) - xAlg(8*par.N+1:9*par.N);    %difference in temperatures of hot and cold streams at outlet

deltaT_El1 = xDiff(1)-xAlg(7*par.N+1);
deltaT_El2 = xDiff(2)-xAlg(7*par.N+2);
deltaT_El3 = xDiff(3)-xAlg(8*par.N);

% g = {g{:},eqnAlg, eqnDiff, Iden,eqnPnet,deltaT_El1,deltaT_El2,deltaT_El3};
% lbg = [lbg;zeros(12*par.N,1);IdenMin*ones(par.N,1);0;0;0;0];
% ubg = [ubg;zeros(12*par.N,1);IdenMax*ones(par.N,1);P0;30;30;30];
...
for nEl = 1:par.N
    ...
    T_el_in = [T_el_in res(7*par.N+nEl)];         %temp of inlet lye stream coming into the electrolyzer
    T_CW_out = [T_CW_out res(8*par.N+nEl)];
    
    %optimal value of the differential state
    Tk = [Tk res(9*par.N+nEl)];               %temperature of the individual electrolyzer
    massBt = [massBt res(10*par.N+nEl)];
    T_bt_out = [T_bt_out res(11*par.N+nEl)];
    
    %optimal value of the inputs
    Vss = [Vss res(12*par.N+nEl)];            %electrolyzer voltage
    q_lyek = [q_lyek res(13*par.N+nEl)];      %lye flowrate
    Qcoolk = [Qcoolk res(14*par.N+nEl)];
...
z0 = [Uk Ik Pk Feffk nH2k qH2Olossk T_bt_in T_el_in T_CW_out];
x0 = [Tk massBt T_bt_out];
u0 = [Vss q_lyek Qcoolk Qwater];
