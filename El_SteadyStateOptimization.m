...
% lbq_lye_k = 6.35e3*ones(par.N,1);%fixed lye flowrate for DegHex
% ubq_lye_k = 6.35e3*ones(par.N,1);
% lbq_lye_k = 6.87e3*ones(par.N,1);%fixed lye flowrate for NewHex
% ubq_lye_k = 6.87e3*ones(par.N,1);
% lbq_lye_k = 6.43e3*ones(par.N,1);%fixed lye flowrate for OSHex
% ubq_lye_k = 6.43e3*ones(par.N,1);
...
Pcons = sum(PconsEl);
qlyeTot = sum(qlyeEl);
J = -(Objvol_H2(1)+Objvol_H2(2)+Objvol_H2(3));%-1e-4*qlyeTot;%+0.0001*Pcons;
