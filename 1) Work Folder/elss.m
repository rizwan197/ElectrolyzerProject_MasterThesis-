...
for nEl = 1:par.N
    ...
    P = [P x(3*par.N+nEl)];         %power of the individual electrolyzer
end
...
for nEl = 1:par.N
    u0(nEl) = param.u0;                       %initial guess for cell voltage
    P0(nEl) = param.P0;
    q_lye0(nEl) = param.q_lye(nEl);
    i0(nEl) = P0(nEl)/(par.EL(nEl).nc*u0(nEl));                %initial guess for current
    T_k0(nEl) = 80;
...
for nEl = 1:par.N
    ...
    par.EL(nEl).Urev = 1.5184 - 1.5421e-3*(273+T_k(nEl)) + 9.523e-5*(273+T_k(nEl))*log((273+T_k(nEl))) + ...
        9.84e-8*(273+T_k(nEl))^2; %relation for Urev with T from LeRoy eqn. 58
    
    eqn(nEl) = u(nEl)*i(nEl)*par.EL(nEl).nc-P(nEl);                         %power = nc*UI
    ...
    eqn(2*par.N+nEl) = q_lye0(nEl)*par.Const.CpLye*(T_in-T_k(nEl)) + par.EL(nEl).nc*(u(nEl)-par.EL(nEl).Utn)*i(nEl) - ...
        par.TherMo(nEl).A_surf*i(nEl)* par.EL(nEl).nc/1000*(par.TherMo(nEl).hc*(T_k(nEl)-par.EL(nEl).Ta) + ...
        par.sigma*par.em*((T_k(nEl)+273.15)^4-(par.EL(nEl).Ta+273.15)^4));  %temperature of each electrolyzer
end
for nEl = 1:(par.N-1)
    eqn(3*par.N+nEl) = u(nEl)*par.EL(nEl).nc - u(nEl+1)*par.EL(nEl+1).nc;   %U(j)*nc(j)= U(j+1)*nc(j+1)
end
...
eqn(4*par.N) = sumP - sum(P0);                                              %Power required = Initial power
...
V_H2_ini = nH2el_ini*0.0224136*3600;%[Nm3/h]
for nEl = 1:par.N
    Ps_ini(nEl) = (U(nEl)*I(nEl)*par.EL(nEl).nc)/(1000*V_H2_ini(nEl));%[kWh/Nm3]
end
% V_H2_ini;
% Ps_ini;

%Calculation of nominal cooling water flow rate and deltaT_LM

Tw_out = 20;                %assuming nominal cooling water delta T to be 10 degC
deltaT1 = T_in - par.Tw_in;     %temperature difference between hot and cold streams at inlet
deltaT2 = T_out - Tw_out;   %temperature difference between hot and cold streams at outlet
deltaT_LM = (deltaT1-deltaT2)/log(deltaT1/deltaT2); %nominal LMTD

q_cw = Qcool/(par.Const.Cp*(Tw_out-par.Tw_in));%nominal cooling water flowrate [g/s]
UA_hex = Qcool/deltaT_LM;%UA of the heat exchanger, [Watt/K]

% for i=1:N
% Qgen(i) = nc(i)*(U(i)-Utn(i))*I(i)
% end

z0 = [u_ini, i_ini, Pk, Feff_ini, nH2el_ini, qH2O_loss, nH2out_ini, nO2el_ini, nO2out_ini, T_out, Tw_out];
x0 = [Tk_ini, Qcool, V_ini, q_cw UA_hex];

% z0s = [u_ini, i_ini, Pk, Feff_ini, nH2el_ini, qH2O_loss, nH2out_ini, nO2el_ini, nO2out_ini, T_out Tw_out]; (7N4)
% x0s = [Tk_ini, Qcool, V_ini, q_cw, UA_hex];
