function [solver,MPC] = GenericDirectCollocation_OCP(par,MPCinit)
import casadi.*
 
 
[~,~,~,lbx,lbz,lbu,ubx,ubz,ubu] = Initialization_bounds();

%Getting Initial state and initial guesses from a structure
dx0 = MPCinit.dx0;   %initial guess for Differential State
z0 = MPCinit.z0;    %Initial guess for Algebraic States
u0 = MPCinit.u0;    %                   Manipulated Variables
u_in = MPCinit.u_in;% Previous MV to be used for delU penalisation in objective
...
L = sum((w_gl-w_gl_SP).^2); % Objective term that needs to be minimized
...
U0 = MX.sym('U0',nu);
GOR_est = MX.sym('GOR_est',nu);
PI_est  = MX.sym('PI_est',nu);
Ud   = MX.sym('Ud',nx);
    ...
    for k = 0:par.N-1
        ...
        w0 = [w0;u0];
            ...
            %Collecting into parameter to be sent as inputs to model integrator
            Upar = vertcat(Uk,w_gl_SP,GOR_est,PI_est,Ud);
        ...
        % Add any other constraint here : 
        g = {g{:},Zkj{d}(30)}; % This is the last algebraic
        lbg = [lbg;0];
        ubg = [ubg;par.QgMax];
...
nlp = struct('x',vertcat(w{:}),'p',vertcat(w_gl_SP,GOR_est,PI_est,Ud,X0_par,U0),'f',J,'g',vertcat(g{:}));
 
solver = nlpsol('solver','ipopt',nlp,opts);
 %not solving the NLP here
MPC.w0 = w0;
MPC.lbw = lbw;
MPC.ubw = ubw;
MPC.lbg = lbg;
